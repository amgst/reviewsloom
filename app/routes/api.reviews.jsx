...
export const loader = async ({ request }) => {
  try {
    ...
    const [reviews, reviewSettings] = await Promise.all([
      prisma.review.findMany({ where: { ...(shop ? { shop } : {}), ...(productId ? { productId } : {}), status: "APPROVED" }, orderBy: { createdAt: "desc" }, take: limit, select: { reviewer: true, rating: true, body: true, imageUrl: true, productName: true, createdAt: true } }),
    ...
    return json({ average: Number(average.toFixed(1)), count: reviews.length, reviews, settings }, { headers: { "Access-Control-Allow-Origin": "*", "Cache-Control": "public, max-age=60" } });
  } catch (error) {
